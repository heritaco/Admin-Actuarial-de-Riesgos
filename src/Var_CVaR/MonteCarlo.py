...
def montecarlo(log_returns, alpha=0.01, window=500, days=10, simulaciones=10_000):
    ...
    for t in range(len(log_returns)):
        ...
        # last `window` log_returns including today
        sample = log_returns.iloc[t + 1 - window: t + 1].values
        ...
        # ascending: worst log_returns first
        montecarlo_sorted = np.sort(montecarlo)

        # number of tail observations
        k = int(np.ceil(alpha * simulaciones))
        # safety: at least 1
        k = max(k, 1)

        # VaR threshold return is the k-th worst return (0-indexed -> k-1)
        # (approximately) alpha-quantile
        q_alpha = montecarlo_sorted[k - 1]

        VaR = -q_alpha                                         # positive loss number
        # mean of worst k log_returns (positive loss)
        CVaR = -montecarlo_sorted[:k - 1].mean()
